...
class ChatbotManager {
  ...
  setupChatbot() {
    try {
      ...
      this.elements = {
        ...
        typingIndicator: document.querySelector(
          Selectors.chatbot.typingIndicator
        ),
      };
      ...
      if (!this.elements.container) {
        console.warn(
          "⚠️ Conteneur de chatbot non trouvé, le chatbot ne sera pas initialisé"
        );
        return;
      }
  ...
  setupEventListeners() {
    ...
    document.addEventListener(Events.chatbot.open, () =>
      this.handleExternalOpen()
    );
    document.addEventListener(Events.chatbot.close, () =>
  ...
  renderMessage(message) {
    ...
    const messageElement = document.createElement("div");
    messageElement.className = `${Classes.chatbot.message} ${
      message.type === "user"
        ? Classes.chatbot.userMessage
        : Classes.chatbot.botMessage
    }`;
  ...
  setAutoCloseDelay(delay) {
    ...
    console.log(
      `⏱️ Délai de fermeture automatique défini à ${this.autoCloseDelay}ms`
    );
  }
